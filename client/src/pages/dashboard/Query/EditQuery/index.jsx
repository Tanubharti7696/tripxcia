import {
  ...
  import { CabFirstStap, FindQuerybYid, FlightFirstStep, HotelFirstStap, SaveCab, SaveFlight, SaveHotel } from "@/data/apis";
...
useEffect(()=>{
  ...
  .then((response)=>{
      ...
      setdata({...data,
        ...
        service:response.result.serviceType,
        cabBookingType:response?.result?.cabBookingType,
        city:response?.result?.city,
        DomesticOrInternational:response?.result?.domesticOrInternational,
        hotelName:response?.result?.hotelName,
        checkInDate:response?.result?.checkInDate,
        checkOutDate:response?.result?.checkOutDate,
             noOfNights:response?.result?.noOfNights,
             mealPlan:response?.result?.mealPlan,
             hotelCategory:response?.result?.hotelCategory,
             roomOcuppency:response?.result?.roomOcuppency,
             noOfRooms:response?.result?.noOfRooms,
              noOfGuests:response?.result?.noOfGuests,
              noOfAdults:response?.result?.noOfAdults,
              noOfChildren6:response?.result?.noOfChildren6,
              noOfChildren12:response?.result?.noOfChildren12,
              tripStartDateTime:response?.result?.tripStartDateTime,
      tripEndDateTime:response?.result?.tripEndDateTime,
      cabType:response?.result?.cabType,
      totalPassenger:response?.result?.totalPassenger,
      bookingDate:response?.result?.bookingDate,
      passengerNumber:response?.result?.PassengerNumber,
      domesticOrInternational:response?.result?.DomesticOrInternational,
      oneWayOrRoundway:response?.result?.OneWayOrRoundTrip,
      departureDate:response?.result?.DepartureDate,
      returnDate:response?.result?.returnDate,
      departureFrom:response?.result?.departureFrom,
      arrivalTo:response?.result?.arrivalTo,
